...
class Tutorial1():
    ...
    def stop(self):
        flag = True
        while flag:
            gool = self.move_group.get_current_pose().pose
            
            #print("en Y: "+str(gool.position.y))
            #print("en z: "+str(gool.position.z))
            #print("esta en el while")
            if  (gool.position.y < 0.04 and gool.position.y > -0.04)  and   (gool.position.z < 0.92 and gool.position.z > 0.88):
                #print("flag2")
                flag = False
